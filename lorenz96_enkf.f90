...
!  do iens_exp=nens_exp,nens_exp !Number of ensemble
  ...
  !Write enseman/sprd***
  call write_ensmean(nens,nobs,iinf,sigma_loc,it_io,0, &
       & xtin,xin,xfinmean,xnull,xnull,inf_parm(iinf))
  call write_enssprd(nens,nobs,iinf,sigma_loc,it_io,0, &
       & xfinsprd,xnull,inf_parm(iinf))
  ...
  do it=1,nt
     ...
     if(mod(it,1000) == 0 .or. mod(it,nt) == 0) &
        ...
        if(mod(it,nint(int_obs/dt)) == 0)then
           ...
           call write_ensmean(nens,nobs,iinf,sigma_loc,it_io,it, &
                & xtout,xout,xfoutmean,xaoutmean,obserr,inf_parm(iinf))
           call write_enssprd(nens,nobs,iinf,sigma_loc,it_io,it, &
                & xfoutsprd,xaoutsprd,inf_parm(iinf))
     ...
     !---IO
     call write_ens_ave_bias_rmse_sprd(nens,nobs,iinf,sigma_loc, &
